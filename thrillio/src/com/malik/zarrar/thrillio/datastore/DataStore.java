package com.malik.zarrar.thrillio.datastore;
...
import com.malik.zarrar.thirillio.managers.MovieGenre;
import com.malik.zarrar.thirillio.managers.UserManager;
import com.malik.zarrar.thrillio.constants.BookGenre;
import com.malik.zarrar.thrillio.constants.Gender;
import com.malik.zarrar.thrillio.constants.UserType;
...
import com.malik.zarrar.thrillio.util.IOUtil;
...
public class DataStore {
	...
	private static void loadUsers() {
		 ...
		 * "user4@semanticsquare.com", "test", "Zarrar", "M", Gender.MALE,
	...
	private static void loadMovies() {
		 ...
		 * "Orson Welles" }, new String[] { "Orson Wells" }, MovieGenre.CLASSICS, 8.5);
		 * bookmarks[1][1] = BookmarkManager.getInstance().createMovie(3001,
		 * "The Grapes of Wrath", "", 1940, new String[] { "Henry Fonda",
		 * "Jane Darwell", "John Ford" }, new String[] { "Jane Darwell" },
		 ...
		 * "Mark" }, MovieGenre.DOCUMENTARIES, 7.3); bookmarks[1][3] =
		 * BookmarkManager.getInstance().createMovie(3003, "The Big Bang Theory",
		 * "Samuel", 2007, new String[] { "Kaley Cuoco", "Jim Parsons", "Chuck Lorre",
		 * "Bill Prady" }, new String[] { "Pearson" }, MovieGenre.TV_SHOWS, 8.7);
		 * bookmarks[1][4] = BookmarkManager.getInstance().createMovie(3004, "Ikiru",
		 * "", 1952, new String[] { "Takashi Shimura", "Minoru Chiaki", "Akira Kurosawa"
		 * }, new String[] { "Akira luffu" }, MovieGenre.FOREIGN_MOVIES, 8.4);
		 */
	...
	private static void loadBooks() {
		...
		for (String row : data) {
			...
			bookmarks[2][colNum++] = BookmarkManager.getInstance().createBook(Long.parseLong(values[0]), values[1],
					row, Integer.parseInt(values[2]), values[3], authors, values[5],
					Double.parseDouble(values[6])/* , values[7] */);
		}
	}
